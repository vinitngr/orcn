              ...
              <div>
                ...
                {(container.mounts || []).map((m: any, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
                    <select value={m.volumeName} onChange={(e) => updateArrayItem(cIndex, 'mounts', i, 'volumeName', e.target.value)} style={{ flex: 1, padding: '0.5rem', background: 'var(--bg-color)', border: '1px solid var(--border)', color: 'var(--text-main)' }}>
                      <option value="">Select Disk...</option>
                      {nodeVolumes.map((v: any, vi: number) => <option key={vi} value={v.name}>{v.name} ({v.size}GB)</option>)}
                    </select>
